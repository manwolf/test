...
class userCourseCalendar extends crudCtr {
	...
	function queryCourse() {
		...
		if (! $newrow) {
		    ...
		    on a.user_tid=u.tid  where class_start_date>="' . $beginDate.'" and  u.tid=' . $tid . ' group by c.tid';
			//class_start_date>="' . $beginDate . '" and class_start_date<="' . $endDateOne . '" and
	...
	function userAddClass() {
		...
		$newrow = $capturs ['newrow'];
		$user_tid = $newrow ['user_tid'];
		if($user_tid =='' && $user_tid==null)
		{
			$msg->ResponseMsg ( 1, '请传入学生id', $result, 0, $prefixJS );
			exit;
		}
		$querySql='select user_add_class from user_info where tid='.$user_tid;
		$model = spClass ( $this->tablename );
		$resultqu = $model->findSql ( $querySql );
		if($resultqu[0]['user_add_class'] !=0){
			$msg->ResponseMsg ( 1, '你已申请！请等待客服处理！', $result, 0, $prefixJS );
			exit;
		}
		// $tid = $newrow['tid'];
		// 添加学生加课记录
