...
class SCInvitationCtr extends userInfoCtr {
	...
	function queryInvitationinfo() {
		...
		$querySql = 'SELECT a.tid,a.invitation_name,a.invitation_amount,a.invitation_discount,
				...
				a.invitation_city,	a.invitation_times,	a.create_time,c.user_name AS sc_name,							
				...
				FROM invitation_info  a  
				LEFT JOIN oa_user_info c ON a.sc_admin_tid = c.tid
	...
	function exportFile() {
		...
		switch ($filetype) {
			...
			case "txt" : // txt记事本文件
				...
				header ( "Content-Disposition:attachment;filename= eTeacher邀请码.txt ;charset=utf-8" );
				break;
			default : // 默认excel表格文件
