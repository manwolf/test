...
class payListCtr extends userInfoCtr {
	...
	public function add() {
		...
		// 验证请求参数 和 token
		if (! $this->verifyUserToken($token, $resultquery[0]["tid"]) ) {
	...
	public function updatePayList() {
		...
		if(1 == $pay_list_array [0]['pay_done']){
			exit($prefixJS . "({\"code\":0,\"msg\":\"支付成功!\",\"data\":[{}],\"pages\":0})");
		}
		// 判断钱包是否充值
		...
		if ($pay_list_array [0] ['invitation_code'] && 0 == $pay_list_array [0]['pay_done']) {
			// 修改订单金额
		...
		if ($result [0] ['user_spelling_lesson_tid'] > 0) { // 若有拼课订单		                                                   
			...
			if ($resultqs [0] ['pay_number'] == $resultqs [0] ['participants_number']) { // 支付人数与参与人数相同的话 则拼课生效
	...
	public function payForH5() {
		...
		// 验证token
		$model = spClass ( 'user_info' );
		$Sql = "select tid from user_info where token='" . $token . "' and tid ='" . $order_array [0] ['user_tid'] . "'";
		$token_result = $model->findSql ( $Sql );
		if (! $token_result [0] ['tid']) {
			echo $prefixJS . "({\"code\":1,\"msg\":\"Token error.\",\"data\":[{}],\"pages\":0})";
			return false;
