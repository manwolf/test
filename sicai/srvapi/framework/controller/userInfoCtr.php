...
class userInfoCtr extends crudCtr {
	...
	public function recordSCIdentifier($user_tid = "", $sc_num = "") {			
		if ($sc_num == null or $user_tid == null) {
			...
			$querySql = ' SELECT sc_tid,user_city FROM user_info WHERE tid =  "' . $user_tid . '"';
			$userResult = @$model->findSql ( $querySql );
			if ($userResult['0']['sc_tid']) {
				return "用户不能重复记录市场人员编号";//false;
			}
			//判断该市场人员的编号是否有效，如果不存在，则退出，返回false；
			$model = spClass ( 'oa_user_info' );
			$querySql = ' SELECT tid FROM oa_user_info 
					WHERE city = "'.$userResult['0']['user_city'].'" AND num =  "' . $sc_num . '"';
			...
			$model = spClass ( 'user_info' );
			$updateSql = ' UPDATE user_info SET sc_tid= "'.$scResult['0']['tid'].'" WHERE tid =  "' . $user_tid . '"';
