...
// 			 exit;
	...
	function sysland (){
		...
		$model = spClass ( $this->tablename );
		if ($newrow)  {
			$querySql = 'select tid,sys_name,sys_pwd from sys_admin_info where ';
			foreach ( $newrow as $k => $v )
			{
			...
			$sys_name = $newrow ['sys_name'];
			$sys_pwd = $newrow ['sys_pwd'];
				
			if ($result && ! $sys_name == null && ! $sys_pwd == null )
			{
				$msg->ResponseMsg ( 0, 'success', $result, 0, $prefixJS );
			} else {
				$msg->ResponseMsg ( 1, 'Password is not correct', false, 0, $prefixJS );
			}
		}
		else
		{
