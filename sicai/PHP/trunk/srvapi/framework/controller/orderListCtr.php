...
class orderListCtr extends userInfoCtr {
	...
	function queryOrder() {
		...
		// 查询所有非拼客订单 o.order_type !=2
		$querySql = 'select o.*, t.teacher_name, t.teacher_image, c.class_count, c.class_disc,u.spelling_lesson_number,
				    u.participants_number,s.class_count as spell_class_count
                    from order_list o left join teacher_info t on o.teacher_tid=t.tid left join class_discount c
				     ...
				     left join  high_quality_courses h 
				    where o.order_state != "2" and  o.order_type !=2  and o.order_type !=3 and o.user_tid=' . $user_tid . ' group by tid order by create_time ';
		...
		// 查询拼客所有已支付的订单 区分拼课与其他订单order_type为3时为拼课
		$querySql = 'select o.*, t.teacher_name, t.teacher_image, c.class_count, c.class_disc,u.spelling_lesson_number,
				    u.participants_number,s.class_count as spell_class_count
                    from order_list o left join teacher_info t on o.teacher_tid=t.tid left join class_discount c 
				     on o.class_discount_tid=c.tid left join user_spelling_lesson u on u.tid=o.user_spelling_lesson_tid
				     left join  spelling_lesson_discount s on s.city=t.teacher_city
				    where o.order_state != "2" and  o.order_type=3  and  o.order_type !=2 and o.pay_done=1 and o.user_tid=' . $user_tid . ' group by tid order by create_time ';
		
		$model = spClass ( $this->tablename );
		$resultquery = $model->findSql ( $querySql );
		
		// 将两个json数组合并输出
		$result = array_merge ( $resultqu, $resultquery );
