...
class userSpellingLesson extends crudCtr {
	...
	function userClassSurplusNum() {
		...
		$querySql = 'select count(c.user_confirm)*2  as userClassSurplusNum
					   ...
					   where  u.tid=o.user_tid and o.tid=c.order_tid and c.user_confirm=0 and u.tid=' . $user_tid;
	...
	function spellingLessonInformation() {
		...
		if($quality ==''){
			$msg->ResponseMsg ( 1, '没有获取到拼客类型！', $result, 0, $prefixJS );
			exit;
		}
		// $user_tid=$result[0]['user_tid'];
		// 判断该学生是否已经 参与拼客 或者 发起拼客
		...
		if (0 == $resulqu [0] ['spelling_class_state']) { // 0表示该学生目前没有参与或发起拼客
			...
			if (count ( $result ) > 0) {
				...
				if($class_quality ==''){
					$msg->ResponseMsg ( 1, '没有获取到拼客类型！class_quality', $result, 0, $prefixJS );
					exit;
				}
				//如果发起普通拼客
				if(0==$class_quality){
					// 根据拼客信息 给发起人生成一张未支付订单    class_types默认为0 标记为普通
				...
				//如果发起精品拼客
				if(1==$class_quality){
					// 根据拼客信息 给发起人生成一张未支付订单 class_types=1 标记为精品  没有老师  
			...
			$msg->ResponseMsg ( 1, '您正在拼客中，不能重复拼客！', $result, 0, $prefixJS );
		}
		return true;
	}
	...
	function query() {
		...
		switch ($participants_number) {
			...
			case $participants_number : 
				//当拼客为普通课时(class_quality: 0为普通拼客  1为精品拼客 必传字段)
				if($class_quality ==''){
				...
				if(0==$class_quality){
					...
					if(!$resultd){
						$msg->ResponseMsg ( 1, '没有获取到普通拼客的折扣及课时数信息！', $result, 0, $prefixJS );
						exit;
					}
					...
					// 不能以学生的身份进入 否则只能看到该学生的信息 根据拼客信息查询所有拼客 参与者的信息
				...
				if(1==$class_quality){
					if($high_quality_courses_tid == ''){
						$msg->ResponseMsg ( 1, '不能获取拼客信息！', $result, 0, $prefixJS );
						exit;
					}
					...
					if(!$resulquer){
						$msg->ResponseMsg ( 1, '没有获取到精品拼课的价格或者折扣信息！', $result, 0, $prefixJS );
						exit;
					}
					//原价
					...
					$querySql = 'select distinct u.tid as user_tid,l.tid as user_spelling_lesson_tid,o.tid as order_tid,l.course_package,o.order_name,u.user_image,l.spelling_lesson_number,l.participants_number,
	    		         ...
	    		         from user_info u inner join order_list o on u.tid=o.user_tid inner join
	    		         user_spelling_lesson l  on o.user_spelling_lesson_tid=l.tid inner join high_quality_courses h
	...
	function addSpellingLesson() {
		...
		$class_quality=$newrow['class_quality']; //拼客类型  0普通  1精品
		$high_quality_courses_tid=$newrow['high_quality_courses_tid'];
...
// 				$result = $model->findSql ( $addSql );
				...
				$msg->ResponseMsg ( 1, '您已发起或正在参与拼课，请不要重复拼客！', $result, 0, $prefixJS );
			}
		} else {
	...
	function cancelSpellingLesson() {
		...
		if (0 == $result [0] ['spelling_lesson_type']) {
			...
			if ($result [0] ['participants_number'] <= 1 && 0 == $result [0] ['pay_done']) {
				...
				// 每人只能参与一个拼客 当发起人取消时 又可以发起或参与新的拼客
            ...
            elseif (1 == $result [0] ['spelling_lesson_type']) {
			      ...
			      $querySql = 'select o.user_tid,o.spelling_lesson_type,o.pay_done from order_list o,user_spelling_lesson s
			                   ...
			                   // exit;
			                   if (0 == $result [0] ['pay_done']) { // 发起人未支付 参与者可以取消拼客
				               ...
				               // 每人只能参与一个拼客 当参与者取消时 又可以发起或参与新的拼客
				               $update = 'update user_info set  spelling_class_state=0 where tid=' . $user_tid;
				               $model = spClass ( $this->tablename );
				               $resultup = $model->runSql ( $update );
				               $msg->ResponseMsg ( 0, 'secsess', $result, 0, $prefixJS );
