...
function template_outputfilter_trimwhitespace($tpl_source, &$template_object)
{
    ...
    $tpl_source = preg_replace("!<script[^>]+>.*?</script>!is", '@@@TEMPLATELITE:TRIM:SCRIPT@@@', $tpl_source);
    
    // Pull out the pre blocks
    preg_match_all("!<pre>.*?</pre>!is", $tpl_source, $match);
    $_pre_blocks = $match[0];
    $tpl_source = preg_replace("!<pre>.*?</pre>!is", '@@@TEMPLATELITE:TRIM:PRE@@@', $tpl_source);
    
    // Pull out the textarea blocks
    preg_match_all("!<textarea[^>]+>.*?</textarea>!is", $tpl_source, $match);
    $_textarea_blocks = $match[0];
    $tpl_source = preg_replace("!<textarea[^>]+>.*?</textarea>!is", '@@@TEMPLATELITE:TRIM:TEXTAREA@@@', $tpl_source);
