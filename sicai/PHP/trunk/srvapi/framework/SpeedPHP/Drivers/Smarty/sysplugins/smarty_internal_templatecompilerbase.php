...
class Smarty_Internal_TemplateCompilerBase
{
                        ...
                        if ($type == Smarty::PLUGIN_COMPILER) {
                            ...
                            if (! is_array($function)) {
                                ...
                                if (is_object($function[0])) {
                                    ...
                                    return call_user_func_array($this->smarty->registered_plugins[$type][$tag][0], array(
                                        $new_args,
                                        $this
                                    ));
                                }
     ...
     * @param boolean $tag_nocache
     *            true if the parser detected a nocache situation
    ...
    public function processNocacheCode($content, $is_code)
    {
        ...
        if ($is_code && ! empty($content)) {
            ...
            if ((! $this->template->resource_object->isEvaluated || $this->template->forceNocache) && $this->template->caching && ! $this->suppressNocacheProcessing && ($this->nocache || $this->tag_nocache || $this->template->forceNocache == 2)) {
