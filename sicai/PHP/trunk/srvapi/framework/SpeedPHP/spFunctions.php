...
function import($sfilename, $auto_search = TRUE, $auto_error = FALSE)
{
    ...
    if (TRUE == @is_readable($sfilename)) {
        ...
        if (TRUE == $auto_search) { // 需要搜索文件
            ...
            foreach (array_merge($GLOBALS['G_SP']['include_path'], array(
                $GLOBALS['G_SP']['model_path']
            ), $GLOBALS['G_SP']['sp_include_path']) as $sp_include_path) {
                // 检查当前搜索路径中，该文件是否已经载入
 ...
 *
 * @param
 *            configname 扩展程序设置点名称
 * @param
 *            launchargs 扩展参数
 * @param
 *            return 是否存在返回数据，如需要返回，则该扩展点仅能有一个扩展操作
 */
...
if (! function_exists('json_decode')) {
    ...
    {
        if ($assoc) {
            return spClass("Services_JSON", array(
                16
            ))->decode($content);
        } else {
